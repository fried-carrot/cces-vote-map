...
PLACE = ["state_lean"]                    # target-encoded state Democratic lean
...
# ---- place feature: state Democratic lean, computed on TRAIN only ------------
...
# shrink small states toward the national mean (empirical-Bayes smoothing)
K = 500.0
state_lean = ((g * n + nat * K) / (n + K)).to_dict()
for d in (train, test):
    d["state_lean"] = d["state"].map(state_lean).fillna(nat)
...
def build_X(d):
    ...
    for c in NUMERIC:
        ...
        names.append(c)
    v = d["state_lean"]
    parts.append(((v - v.mean()) / (v.std() + 1e-9)).values)  # standardize place too
    names.append("state_lean")
...
# place standardization params (needed for JS)
place_mean = float(train["state_lean"].mean())
place_std = float(train["state_lean"].std() + 1e-9)
...
# ---- baselines to beat -------------------------------------------------------
report("baseline: state-lean only", test["state_lean"].values)
...
# ---- per-state population prediction (map default layer) ---------------------
# mean predicted P(Dem) over each state's 2024 respondents = MRP-lite reality map
latest = df[df["year"] == 2024].copy()
latest["state_lean"] = latest["state"].map(state_lean).fillna(nat)
Xl, _ = build_X(latest)
...
state_n = latest.groupby("state").size().to_dict()
...
export = {
    "meta": {
        ...
        "note": "self-reported party/ideology excluded; state lean is the place-based signal",
    },
    ...
    "state_pred": state_pred, "state_n": state_n,
